...
RSpec.describe CanonicalLog::Sampling do
  ...
  describe '.sample?' do
    it 'returns true for 500 status' do
      expect(described_class.sample?({ http_status: 500 }, config)).to be true
    end

    it 'returns true for 503 status' do
      expect(described_class.sample?({ http_status: 503 }, config)).to be true
    end
    ...
    it 'returns true when error key is present regardless of status' do
      ...
      expect(described_class.sample?(event, config)).to be true
    end
    ...
    it 'always rejects when sample_rate is 0.0' do
      ...
      event = { http_status: 200, duration_ms: 10 }
      expect(described_class.sample?(event, config)).to be false
    end

    it 'treats missing http_status as 0' do
      config.sample_rate = 0.0
      expect(described_class.sample?({}, config)).to be false
    end

    it 'treats missing duration_ms as 0' do
      config.sample_rate = 0.0
      expect(described_class.sample?({ http_status: 200 }, config)).to be false
    end
  end
end
